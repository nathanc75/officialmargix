      ...
      <Tabs defaultValue="promos" className="w-full">
        <CardHeader className="p-0 border-b bg-muted/30">
          <TabsList className="w-full justify-start h-14 bg-transparent p-0 rounded-none">
            <TabsTrigger 
              value="promos" 
              className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none h-14 px-8 text-sm font-bold uppercase tracking-wider"
            >
              Promo Performance
            </TabsTrigger>
            <TabsTrigger 
              value="ai" 
              className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none h-14 px-8 text-sm font-bold uppercase tracking-wider gap-3"
            >
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className="space-y-1">
                      <p className="font-bold text-base">{promo.name}</p>
                      <Badge variant="outline" className="text-xs font-bold uppercase border-0 bg-muted px-2 py-0.5">
                        {promo.platform}
                      </Badge>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-6 pt-4 border-t border-dashed">
                    <div>
                      <p className="text-xs font-bold text-muted-foreground uppercase tracking-tight">Cost</p>
                      <p className="text-sm font-bold">{promo.cost}</p>
                    </div>
                    <div>
                      <p className="text-xs font-bold text-muted-foreground uppercase tracking-tight">Revenue</p>
                      <p className="text-sm font-bold">{promo.revenue}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="p-5 rounded-xl bg-amber-500/5 border border-amber-500/10">
              <div className="flex items-center gap-4">
                ...
                <p className="text-sm font-semibold text-amber-900 dark:text-amber-100">
                  <strong>Revenue Alert:</strong> 2 promos are underperforming, resulting in <span className="font-bold text-red-600 text-base">$250/week</span> in addressable losses.
                </p>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="ai" className="mt-0 space-y-4">
            <div className="space-y-3">
              {recommendations.map((rec, index) => (
                <div 
                  key={index} 
                  className="p-5 rounded-xl bg-card border shadow-sm hover:shadow-md transition-all duration-200 group"
                >
                  <div className="flex items-start justify-between gap-6">
                    <div className="flex items-start gap-5">
                      <div className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center flex-shrink-0 border">
                        <rec.icon className="h-6 w-6 text-primary" />
                      </div>
                      <div className="space-y-1.5">
                        <p className="font-bold text-base leading-none">{rec.action}</p>
                        <p className="text-sm text-muted-foreground font-medium leading-relaxed">{rec.reason}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <Badge variant="secondary" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 font-bold text-xs uppercase h-8 px-3">
                        {rec.impact}
                      </Badge>
            ...
            <div className="p-5 rounded-xl bg-primary/5 border border-primary/10">
              <div className="flex items-center gap-4">
                ...
                <p className="text-sm font-semibold text-foreground">
                  <strong>Recovery Target:</strong> Implementing these suggestions could recover <span className="font-bold text-primary text-base">+$375/week</span> in pure profit.
                </p>
              </div>
            </div>
          </TabsContent>
